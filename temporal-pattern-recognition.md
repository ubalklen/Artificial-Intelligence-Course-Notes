...
We assume the process is **stationary**, i.e there is a single conditional probability table that holds the values of $P(X_t | X_{t-1})$ and is used for every random variable in the model. The process is called stationary because conditional probabilities are the same regardless of the time $t$.
...
In such a model, we can suggest the next word in a text (a common feature of text processors) by answering "Given the sentence $W = w_1, w_2, \ldots, w_n$, does what word $w_{n+1}$ produce the sentence $W^* = w_1, w_2, \ldots, w_n, w_{n+1}$ with the highest $P(W^*)$?".
...
$$A=\begin{bmatrix}a_{11}&\dots&a_{1n}\\\vdots&\ddots&\\a_{n1}&&a_{nn}\end{bmatrix},\quad a_{ij}=P(X_t=S_j|X_{t-1}=S_i)$$
